            ...
            HStack(spacing: 8) {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    ...
                    HStack(spacing: 6) {
                        ...
                        // Rules 592–593. Uses the printing-aware check, not
                        // the identity-free `CGRect.cardStance` fallback —
                        // this row already knows which card it is, so a
                        // landscape Battlefield isn't misread as tapped.
                        if entry.printing.isExhausted(observedBoundingBox: entry.detection.boundingBox) {
